...
function TabAllocator(browser, maxtabs)
{
  ...
  {
    delete this._tab;
    // RemoveAllTabsBut makes it asynchronously and removes as well new tabs
    // created after that call.
    for (let itab of this._browser.tabs)
      if (tab != itab)
        this._browser.removeTab(itab);
    return tab;
  });
}
TabAllocator.prototype = {
  ...
  getTab: function()
  {
    ...
    // becomes resolved (thus removing of other tabs is finished).
    if (this._tab)
    {
      if (!this._tabInUse)
      {
